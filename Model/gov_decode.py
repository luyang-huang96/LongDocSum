...
def initialize_bart(model_path, data_path, **kwargs):
    bart = LongBartModel.from_pretrained(
        model_path,
    ...
    for split in ['test']:
        with open(os.path.join(data_path, split + '.source')) as source, open(os.path.join(save_path, split + '.hypo'), 'w') as fout:
            ...
            for sline in source:
                if count % bsz == 0:
                    with torch.no_grad():
                        hypotheses_batch = bart.sample(slines, beam=4, lenpen=2, max_len_b=1024, min_len=0, no_repeat_ngram_size=3)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--batch_size', type=int, default=16, help='gpu id')
